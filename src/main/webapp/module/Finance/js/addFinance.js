...
$.ajax({
    url: `${[test[0].url]}/ocrtraining/get?supplier=${$("#vendor_code").val()}`,
    async: false,
    headers: {
        Authorization: `Bearer ${token}`,
    },
...
console.log("receipt and reciepts : " , receipt , receipts);
...
if(receipt.length > 0 || receipts != null)
{
	...
	if(obj.transactionType == "ServiceWithMaterial")
	  ...
	  console.log("uniqueArray    :   " , uniqueArray  , loop_length);
	...
	$("#comp_search").click(()=>{
		...
		$.ajax ({
				...
				$("#fetch_btn").click(()=>{
					...
					// if(obj.transactionType == "Service_PO" || obj.transactionType == "Credit_Note" || obj.transactionType == "Debit_Note")
					// {
					// 	$(".Sacnned_qty").parents("td").addClass("d-none")
					// 	$(".short_quantity").parents("td").addClass("d-none")
					// }
					...
					for(let j = 0 ; j < obj.details.length ; j++)
					{
						...
						if($(".po_item")[i].value == obj.details[j].jdeItemCode)
						{
							...
							try {
								...
								// $(".Sacnned_qty")[i].value = obj.details[j]?.quantity != null ? obj.details[j]?.quantity : obj?.weight ;
								// let scanned_values1 = $(".Sacnned_qty").val().split(" ")[0]
								// let arr2 = scanned_values1.split(/[,]/).join("")
								// $(".short_quantity")[i].value = (+arr2) - (+$(".accept_quantity")[i].value)
				...
				$("#reject_invoice").click((e)=>{
					...
					$("#back_invoice").click((e) => {
						...
						if(obj.voucher_number){
							...
							window.open("../template/finance.jsp", "_self")
