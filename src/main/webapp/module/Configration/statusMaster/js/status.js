    ...
    $("#Dtable").on("click", ".delete", function () {
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `${[test[0].url]}/status/delete/${row}`,
                    type: "delete",
                    dataSrc: "data",
                    headers: {
                        'Authorization': 'Bearer ' + token,
        ...
        $.ajax({
            ...
            success: function (data,status,xhr) {
                if(xhr.status == 200)
                {
                    ...
                    window.location.href = `../template/updateStatus.jsp`;
                }
                else{
    ...
    $("#Dtable").on("click", ".view", function () {
        ...
        $.ajax({
            ...
            success: function (data,status,xhr) {
                ...
                if(xhr.status == 200)
                {
                    $("#code").val(data.data.code).css("font-weight", "bold");
                    $("#description").val(data.data.description).css("font-weight", "bold");
                    $("#label").val(data.data.label).css("font-weight", "bold");
                }
                else{
