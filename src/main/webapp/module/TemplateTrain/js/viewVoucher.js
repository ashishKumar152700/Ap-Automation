...
var invoice_dates = $("#invoice_date").val(data.data[0].invoice_date);
var GL_dates = $("#gl_date").val(data.data[0].gl_date != null ? data.data[0].gl_date  : (new Date()).toISOString().split('T')[0]);
...
$("#gl_date").val(data.data[0].gl_date != null ? data.data[0].gl_date  : new Date().toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' }).split("/").reverse().join("-"));
...
if(receipt.length > 0 || receipts != null)
{
	...
	$.ajax({
	url: `${[login[0].url]}/jderest/v2/dataservice/table/F43121?$field=F43121.DOC&$field=F43121.LOCN&$field=F43121.GLC&$field=F43121.URDT&$field=F43121.DCT&$field=F43121.UREC&$field=F43121.AOPN&$field=F43121.DOCO&$field=F43121.LITM&$filter=F43121.MATC EQ 1&$filter=F43121.MCU EQ ${$("#business_unit").val()}&$field=F43121.MCU&$filter=F43121.CNID EQ ${$("#invoice_noo").val()}&$filter=F43121.DCT EQ OV&$field=F43121.AREC&$field=F43121.VRMK&$field=F43121.CNID&$filter=F43121.DOCO EQ ${ loop_length > 1 ? uniqueArray[i].po_number : $("#purchase_order").val()}&$filter=F43121.DCTO EQ ${ loop_length > 1 ? uniqueArray[i].po_type : $("#purchase_type").val()}`,
	// type : 'POST',
						...
						//     {
								...
								$.ajax({
										...
										success: function (data) {
											...
											// $.ajax({
											//     url : `${[test[0].url]}/gate/getAll?id=${gateNumber}`,
											//     success : function(data){
											//         // console.log(data);
											...
											//         // vehicle_nbr = data.data[0].vehicle_nbr 
											//         // material_type = data.data[0].material_type 
											//         // weight = data.data[0].weight
											//         // in_time = data.data[0].in_time 
											//         // division = data.data.division 
											//         // remark = data.data[0].remark 
											//         // vendorname = data.data[0].vendorname
											...
											if (arr.flat(5000).includes(Purchase_order) == false && data.amount != 0 && (overAllCurrency.length == 0 || overAllCurrency.includes(data.currency_code) == true)) {
													...
													for (let i = (count_len == 0 ? count_len : count_len ); i < (count_len == 0 ? count_len + m : (count_len + 1) + m) ; i++)
														{
															...
															// console.log(data.business_unit);
															// $("#business_unit").val($("#business_unit").val().trim())
												...
												for (let i = 0; i < (count == 0 && $(validate).val() == "" ? data.po_details.length : data.po_details.length + 5); i++)
												// for(let i = 0 ; i <     ; i++)
												{
												...
												// for(let p = 0 ; p < obj.details.length ; p++)
												// {
													...
													$.ajax({
														...
														success: function (data) {
															...
															for (let i = count ; i < data.po_details.length  ; i++)
															{
																...
																<td class="text-center MSacnned_qty"></td>
																<td class="text-center Maccept_quantity"></td>
																<td class="text-center Mshort_quantity"></td>
															...
															$("#modal_table_body").append(modalRow)
																...
																console.log("data details " , data.po_details[i].item_number);
																// console.log("match item " , Matched_item.flat(5000));
																// console.log_item.flat(5000).includes(data.po_details[j].item_number));

																if(Matched_item.flat(5000).includes(data.po_details[i].item_number))
																{
														...
														if(obj.transactionType == "Service_PO")
														{
															$(".Sacnned_qty").parents("td").addClass("d-none")
															$(".short_quantity").parents("td").addClass("d-none")
														}
		...
		else
		{
		  ...
		  $("#myModal18").on('hide.bs.modal', function () {
			...
			$("#create_voucher").click(()=>{
				...
				$.ajax({
					...
					success : function(data ,  status , xhr)
					{
						if(xhr.status == 200){
							...
							$.ajax({
								url : `${[login[0].url]}/jderest/v2/dataservice/view/V75I411A?$filter=F0411.DOC EQ 23045454&$.filter=F0411.DCT EQ PV&$.filter=F0411.KCO EQ 00210&$.filter=F0411.MCU EQ 20100&$.filter=F0411.AN8 EQ 40000005`,
								headers: {
										'Accept': 'application/json',
										'Content-Type': 'application/json',
										"Authorization": "Basic " + btoa(`${[login[0].username]}` + ":" + `${[login[0].password]}`)
                ...
                //  url: `${[login[0].url]}/jderest/v3/orchestrator/ORCH_AP_GRN`,
                ...
                //          console.log(xhr.responseJSON.message.jde__simpleMessage);
				...
				$("#submit_invoice").click((e)=>{
					...
					if($("textarea").val().length != 0)
					{
						...
						let receipt_doc =  obj.details.map((value)=> value.receipt_number)

						if(!receipt_doc.includes(null))
						{
