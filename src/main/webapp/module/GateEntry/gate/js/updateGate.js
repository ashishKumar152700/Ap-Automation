              ...
              if (res.status != 500) {
                ...
                if (loopCount != 0) {
                  for (let i = 1; i <= loopCount; i++) {
                    ...
                    console.log("supplier : ", supplier_code);
                    console.log("i : ", i);
    
                    await fetch(
                      `${[
                        test[0].url,
                      ]}/ocrtraining/get?supplier=${supplier_code}&template=Template_${i}`,
                      {
                        headers: {
                          Authorization: "Bearer " + token,
                        },
                    ...
                    if (flag) {
                      ...
                      for (let i = 0; i < req_body.details.length; i++) {
                        ...
                        $("#tab_logic_body").append(`<tr>
                                        ...
                                        <td><input type="text" class="form-control input_size quantity text-center" readonly value="${req_body.details[i].cgstAmount}"></td>
                                        <td><input type="text" class="form-control input_size quantity text-center" readonly value="${req_body.details[i].cgstPercentage}"></td>
                                        <td><input type="text" class="form-control input_size quantity text-center" readonly value="${req_body.details[i].sgstAmount}"></td>
                                        <td><input type="text" class="form-control input_size quantity text-center" readonly value="${req_body.details[i].sgstPercentage}"></td>
                                        <td><input type="text" class="form-control input_size quantity text-center" readonly value="${req_body.details[i].igstAmount}"></td>
                                        <td><input type="text" class="form-control input_size quantity text-center" readonly value="${req_body.details[i].igstPercentage}"></td>
                      ...
                      try {
                        fetch(`${[vision[0].url]}/api/getimage`)
  ...
  document.body.appendChild(canvas);
    ...
    $("#save").click((e) => {
      ...
      console.log('else :', amount );
            ...
            $.ajax({
              url : `${[test[0].url]}/validateDate?date=${$("#invoice_date").val()}`,
              headers: {
                'Authorization': 'Bearer ' + token,
              },
              success : function(data,status,xhr)
                          ...
                          $.ajax({
                            ...
                            success: function (data) {
                              ...
                              if (record == 1) {
                                ...
                                //   let dc_number = $("#deliveryChallanNumber").val() != "" ? `&%24filter=F56UD911.YCHL%20EQ%20${$("#deliveryChallanNumber").val()}` : "" ;  
                                ...
                                //                       req_body.status = { code: 1000 };
                                //                       req_body.deliveryChallanNumber = $("#deliveryChallanNumber").val()
                                //                       req_body.workOrderNumber = $("#workOrderNumber").val()
        
                                //                       if($("#deliveryChallanNumber").val() != null)
                                //                       {
                                    ...
                                    error: function (xhr) {
                                      ...
                                      if (xhr.status < 500) {
                                        swal("", xhr.responseJSON.message.DREQ_PO_Header.Message, "error").then(() => {
                                          ...
                                          req_body.error_message = xhr.responseJSON.message.DREQ_PO_Header.Message;
                                          req_body.status = { code: 1000 };
                                          req_body.deliveryChallanNumber = $("#deliveryChallanNumber").val()
                                          req_body.workOrderNumber = $("#workOrderNumber").val()
